...
describe ObservationsImporter, type: :importer do
  ...
  let(:uploaded_file) { fixture_file_upload("data_import/#{importer_type}/import_data.csv", Mime[:csv].to_s) }
  let(:results) { fixture_file_upload("data_import/#{importer_type}/results.json", Mime[:json].to_s).read }
