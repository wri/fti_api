...
RSpec.describe "Admin: Visible Columns and Charts", type: :system do
  ...
  it "should sync columns and chart visibility" do
    ...
    expect(page).to have_checked_field("valid_count")
    expect(page).to have_selector("th.col-valid_count", visible: true)
    expect(get_legend_item("Valid")["hidden"]).to eq(false)
    page.uncheck("valid_count")
    expect(page).not_to have_checked_field("valid_count")
    expect(page).not_to have_selector("th.col-valid_count", visible: true)
