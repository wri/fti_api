...
RSpec.describe APIKey, type: :model do
  subject(:api_key) { FactoryGirl.build :api_key }
  ...
  context 'Methods' do
    context '#expired?' do
      context 'when APIKey expires_at date is lower than current date' do
        it 'returns true' do
          api_key = FactoryGirl.create(:api_key, expires_at: Date.yesterday)
      ...
      context 'when user is deactivated' do
        it 'returns true' do
          ...
          user.deactivate
          api_key = FactoryGirl.build(:api_key, user: user, expires_at: Date.yesterday)
          expect(api_key.expired?).to eql true
        end
      end
