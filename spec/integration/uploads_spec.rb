    ...
    context "download tracking" do
      it "tracks downloads for trackable models with regular browsers" do
        ...
        expect(TrackFileDownloadJob).to have_received(:perform_later).with(
          "test-client-id", request.remote_ip, "direct", nil, request.url, @document_file.attachment.filename, "document_file"
        )
        ...
        expect(response).to have_http_status(:ok)
        expect(TrackFileDownloadJob).to have_received(:perform_later).with(
          "test-client-id", request.remote_ip, "direct", nil, request.url, @observation_report.attachment.filename, "observation_report"
        )
      end
