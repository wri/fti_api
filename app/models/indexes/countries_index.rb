  ...
  private
    ...
    def sort_params
      ...
      if params[:sort].present? && params[:sort].include?('name')
        new_for_sort  = "country_translations.name #{for_sort['name']}"
        new_for_sort += ", countries.updated_at #{for_sort['updated_at']}" if params[:sort].include?('updated_at')
        new_for_sort += ", countries.created_at #{for_sort['created_at']}" if params[:sort].include?('created_at')
