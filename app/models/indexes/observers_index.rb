...
class ObserversIndex
  ...
  private
    ...
    def options_filter
      params.permit('id', 'name', 'sort', 'observer', 'observer' => {}).tap do |filter_params|
        filter_params[:page]= {}
    ...
    def total_pages
      @total_pages ||= observers.total_pages
    end
    ...
    def sort_params
      ...
      if params[:sort].present? && params[:sort].include?('name')
        new_for_sort  = "observer_translations.name #{for_sort['name']}"
        new_for_sort += ", observers.updated_at #{for_sort['updated_at']}" if params[:sort].include?('updated_at')
        new_for_sort += ", observers.created_at #{for_sort['created_at']}" if params[:sort].include?('created_at')
