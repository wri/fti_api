...
class FmusIndex
  ...
  delegate :fmus_url, to: :controller
  ...
  def fmus
    ...
    @fmus_items ||= @fmus.order(sort_params)
                        .paginate(page: current_page, per_page: per_page)
  end

  def total_items
    @total_items ||= @fmus.size
  end

  def links
    {
        first: fmus_url(rebuild_params.merge(first_page)),
        prev:  fmus_url(rebuild_params.merge(prev_page)),
        next:  fmus_url(rebuild_params.merge(next_page)),
        last:  fmus_url(rebuild_params.merge(last_page))
    }
  end
  ...
  def options_filter
    params.permit('id', 'name', 'country', 'sort', 'fmu', 'operator' => {}).tap do |filter_params|
      filter_params[:page]= {}
  ...
  def sort_params
    ...
    if params[:sort].present? && params[:sort].include?('legal_reference')
      new_for_sort  = "fmu_translations.legal_reference #{for_sort['legal_reference']}"
      new_for_sort += ", fmus.updated_at #{for_sort['updated_at']}" if params[:sort].include?('updated_at')
      new_for_sort += ", fmus.created_at #{for_sort['created_at']}" if params[:sort].include?('created_at')
