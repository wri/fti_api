...
class OperatorsIndex
  ...
  delegate :operators_url, to: :controller
  ...
  def links
    {
      first: operators_url(rebuild_params.merge(first_page)),
      prev:  operators_url(rebuild_params.merge(prev_page)),
      next:  operators_url(rebuild_params.merge(next_page)),
      last:  operators_url(rebuild_params.merge(last_page))
    }
  end
  ...
  private
    ...
    def options_filter
      params.permit('id', 'name', 'sort', 'country_ids', 'operator', 'operator' => {}).tap do |filter_params|
        filter_params[:page]= {}
    ...
    def total_pages
      @total_pages ||= operators.total_pages
    end
    ...
    def sort_params
      ...
      if params[:sort].present? && params[:sort].include?('name')
        new_for_sort  = "operator_translations.name #{for_sort['name']}"
        new_for_sort += ", operators.updated_at #{for_sort['updated_at']}" if params[:sort].include?('updated_at')
        new_for_sort += ", operators.created_at #{for_sort['created_at']}" if params[:sort].include?('created_at')
